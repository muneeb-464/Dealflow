        ...
        {notifOpen && (
          <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-2xl shadow-xl border border-neutral/10 overflow-hidden z-50">
            <div className="px-4 py-3 border-b border-neutral/10 flex items-center justify-between">
              ...
              <span className="text-[10px] text-secondary font-semibold bg-secondary/10 px-2 py-0.5 rounded-full">3 new</span>
            </div>
            {[
              { title: "Lead replied", desc: "TechCorp responded to your proposal", time: "2m ago", dot: "bg-secondary" },
              { title: "Follow-up due", desc: "RetailChain B2B — no reply in 48h", time: "1h ago", dot: "bg-tertiary" },
              { title: "New client added", desc: "SaaS Platform onboarded successfully", time: "3h ago", dot: "bg-secondary" },
            ].map((n) => (
              <div key={n.title} className="flex items-start gap-3 px-4 py-3 hover:bg-neutral-light transition-colors cursor-pointer border-b border-neutral/5 last:border-0">
                <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${n.dot}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-primary text-xs font-semibold">{n.title}</p>
                  <p className="text-neutral text-[11px] mt-0.5 truncate">{n.desc}</p>
                </div>
                <span className="text-neutral/50 text-[10px] flex-shrink-0">{n.time}</span>
              </div>
            ))}
            <div className="px-4 py-2.5 text-center">
              <button className="text-xs text-secondary font-semibold hover:text-secondary/80 transition-colors">View all</button>
            </div>
          </div>
        )}
      </div>
